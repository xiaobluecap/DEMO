...
task_number=10
#定义收发队列
task_queue=collections.deque(task_number)
result_queue=collections.deque(task_number)
...
def win_run():
    ...
    #绑定端口并设置验证口令，windows下需要填写ip地址，linux下不填为默认本地
    manger=QueueManger(address=('127.0.0.1',9999),authkey='qiye')
    #启动
    manger.start()
    try:
        #通过网络获取任务队列和结果队列
